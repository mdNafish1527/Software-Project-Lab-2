...
export default function ConcertDetail({ onOpenCart }) {
  ...
  const handleAddToCart = (tier) => {
    ...
    showAlert('success', `✅ ${qty} × ${tier.name} added to cart!`);
  };
                        ...
                        {tiers.map(tier => (
                          <div key={tier.id} style={{ background: 'var(--bg-secondary)', border: 'var(--border-dim)', borderRadius: 'var(--radius-sm)', padding: '16px 20px', minWidth: '150px' }}>
                            ...
                            {cartTicketCount(tier.tierNum) > 0 && (
                              <div style={{ fontFamily: 'var(--text-mono)', fontSize: '10px', color: 'var(--cyan)', marginTop: '4px' }}>
                                🛒 {cartTicketCount(tier.tierNum)} in cart
                              </div>
                            )}
                          </div>
                        ))}
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,212,255,0.2)', borderTop: '2px solid var(--cyan)', borderRadius: 'var(--radius-lg)', padding: '24px', height: 'fit-content' }}>
              ...
              {tiers.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {tiers.map(tier => {
                    const inCart  = cartTicketCount(tier.tierNum);
                    const isAdded = addedTier === tier.tierNum;
                    return (
                      <div key={tier.id} style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: '14px', border: inCart > 0 ? '1px solid rgba(0,212,255,0.3)' : 'var(--border-dim)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                          <button onClick={() => setTierQty(q => ({ ...q, [tier.tierNum]: Math.max(1, q[tier.tierNum] - 1) }))}
                          ...
                          <input type="number" min={1} max={10} value={tierQty[tier.tierNum] || 1}
                            onChange={e => setTierQty(q => ({ ...q, [tier.tierNum]: Math.min(10, Math.max(1, parseInt(e.target.value) || 1)) }))}
                          ...
                          <button onClick={() => setTierQty(q => ({ ...q, [tier.tierNum]: Math.min(10, q[tier.tierNum] + 1) }))}
                            style={{ width: '30px', height: '30px', borderRadius: '6px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: '16px' }}>+</button>
                          <button onClick={() => handleAddToCart(tier)}
                            style={{ flex: 1, background: isAdded ? '#00BFA6' : 'linear-gradient(135deg,#00d4ff,#0099bb)', color: '#000', border: 'none', borderRadius: '8px', padding: '8px', fontWeight: '700', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--text-mono)', letterSpacing: '0.05em', transition: 'background 0.2s' }}>
                            {isAdded ? '✅ Added!' : '+ ADD TO CART'}
                          </button>
                        </div>
                        <div style={{ fontFamily: 'var(--text-mono)', fontSize: '10px', color: 'var(--text-dim)', marginTop: '6px' }}>
                          Subtotal: ৳{((tierQty[tier.tierNum] || 1) * tier.price).toLocaleString()} · max 10
                        </div>
                      </div>
                    );
                  })}
